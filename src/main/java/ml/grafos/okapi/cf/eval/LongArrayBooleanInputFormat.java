 ...
 *
 * |nodeId| |computed model    |    |ids of items in the user's test set|
 * 0 -1
 ...
 *
 */
public class LongArrayBooleanInputFormat extends TextVertexInputFormat<CfLongId, FloatMatrixWritable, BooleanWritable>{
	...
	class LongArrayBooleanVertexReader extends TextVertexReaderFromEachLineProcessed<String[]> {
		...
		protected FloatMatrixWritable getValue(String[] line)
			...
			float[] factorsFloat = new float[factors.length];
			for(int i=0; i<factors.length; i++){
				factorsFloat[i] = Float.parseFloat(factors[i]);
			}
