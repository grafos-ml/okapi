...
public class Sgd extends Vertex<IntWritable, DoubleArrayListHashMapWritable,
  ...
  public void compute(Iterable<MessageWrapper> messages) {
     ...
     * basic, rmse, l2norm
     **/
    String factorFlag = getContext().getConfiguration().get("sgd.factor",
      "basic");
    ...
    if (getSuperstep() == 0
      || (halt_factor > TOLERANCE && getSuperstep() < ITERATIONS)) {
      sendMsgs();
    }
    ...
    public void compute() {
      ...
      if (getSuperstep() > 1) {
        ...
        totalRMSD = Math.sqrt(((DoubleWritable)
          getAggregatedValue(RMSD_AGG)).get() / numRatings);

        System.out.println("Superstep: " + getSuperstep()
          + ", [Aggregator] Added Values: " + getAggregatedValue(RMSD_AGG)
          + " / " + numRatings
          + " = " + ((DoubleWritable) getAggregatedValue(RMSD_AGG)).get()
          / numRatings
          + " --> sqrt(): " + totalRMSD);
