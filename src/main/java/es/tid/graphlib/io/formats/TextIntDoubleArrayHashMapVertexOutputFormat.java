  ...
  protected class TextIntDoubleArrayVertexWriter
    ...
    (Vertex<IntWritable, DoubleArrayListHashMapWritable, IntWritable, ?>
      ...
      boolean flag = getContext().getConfiguration().getBoolean(
        "sgd.printerr", false);
      String type = "";
      if (((SgdGeneralDeltaCaching) vertex).isItem()) {
        // item.concat("item");
        type = "item";
      } else {
        // item.concat("user");
      ...
      String error = null;
      String updates = Integer.toString(((SgdGeneralDeltaCaching) vertex)
        .getUpdates()); // .toString();
      Text line;
      if (flag) {
        error = Double.toString(Math
          .abs(((Sgd) vertex).getHaltFactor()));
